...
type Data struct {
	Zz                string `json:"zz"`
	Ty                string `json:"ty"`
	ZhiyuanSchoolName string `json:"zhiyuan_school_name"`
	MajorCode         string `json:"major_code"`
	Ls                string `json:"ls"`
	Dl                string `json:"dl"`
	Yybs              string `json:"yybs"`
	Whzf              string `json:"whzf"`
	Jrzn              string `json:"jrzn"`
	Ds                string `json:"ds"`
	Hx                string `json:"hx"`
	Wh                string `json:"wh"`
	Kytl              string `json:"kytl"`
	Wl                string `json:"wl"`
	Yswzf             string `json:"yswzf"`
	...
	Tzf               string `json:"tzf"`
	Sw                string `json:"sw"`
	ZhiyuanSchoolCode string `json:"zhiyuan_school_code"`
	Sx                string `json:"sx"`
	...
	BizCode1          string `json:"biz_code1"`
	BizCode3          string `json:"biz_code3"`
	Tdf               string `json:"tdf"`
	Tl                string `json:"tl"`
	Zgdx              string `json:"zgdx"`
	Name              string `json:"name"`
	Zs                string `json:"zs"`
	Jf                string `json:"jf"`
	Yszf              string `json:"yszf"`
}
...
func initResult() {
	...
	// 设置表头
	title := []string{
		"建档号", "班级", "姓名", "录取院校", "总分", "语文", "数学", "英语", "物理", "化学", "道法", "历史", "生物", "地理", "体育", "志愿代码",
	}
...
func (s Student) recognizeCaptcha(r *resty.Client) {
	...
	// 识别验证码
	ocrURL := "http://sanhk3.luckysan.top:9898/ocr/file"
...
func (s Student) getGrade(captcha string, r *resty.Client) {
	...
	case 0: // 获取成绩成功 保存
		resp.saveGrade(s)
		println("总分：" + resp.Data.Jfzf + resp.Data.ZhiyuanSchoolName)
	case 1001: // 验证码错误
...
func (resp Response) saveGrade(stu Student) {
	...
	_ = resultFile.SetCellValue("Sheet1", "A"+i, stu.jdNo)                    // 建档号
	_ = resultFile.SetCellValue("Sheet1", "B"+i, stu.class)                   // 班级
	_ = resultFile.SetCellValue("Sheet1", "C"+i, stu.name)                    // 姓名
	_ = resultFile.SetCellValue("Sheet1", "D"+i, resp.Data.ZhiyuanSchoolName) // 录取院校
	_ = resultFile.SetCellValue("Sheet1", "E"+i, resp.Data.Jfzf)              // 总分
	_ = resultFile.SetCellValue("Sheet1", "F"+i, resp.Data.Yw)                // 语文
	_ = resultFile.SetCellValue("Sheet1", "G"+i, resp.Data.Sx)                // 数学
	_ = resultFile.SetCellValue("Sheet1", "H"+i, resp.Data.Yy)                // 英语
	_ = resultFile.SetCellValue("Sheet1", "I"+i, resp.Data.Wl)                // 物理
	_ = resultFile.SetCellValue("Sheet1", "J"+i, resp.Data.Hx)                // 化学
	_ = resultFile.SetCellValue("Sheet1", "K"+i, resp.Data.Zz)                // 道法
	_ = resultFile.SetCellValue("Sheet1", "L"+i, resp.Data.Ls)                // 历史
	_ = resultFile.SetCellValue("Sheet1", "M"+i, resp.Data.Sw)                // 生物
	_ = resultFile.SetCellValue("Sheet1", "N"+i, resp.Data.Dl)                // 地理
	_ = resultFile.SetCellValue("Sheet1", "O"+i, resp.Data.Ty)                // 体育
	_ = resultFile.SetCellValue("Sheet1", "P"+i, resp.Data.ZhiyuanSchoolCode) // 录取院校志愿代码
	_ = resultFile.Save()
}
